...
/**
 * <p>Title: xCRMS </p>
 * Description: 通过ThreadPoolExecutor创建线程池<br>
    ...
    public static void main(String ...args){
        ...
        IntStream.range(0, 9).forEach(item -> {
            threadPoolExecutor.execute(()->{
                System.out.println(Thread.currentThread().getName()+"Start:"+item + "--" + LocalDateTime.now().toString());
                try {
                    TimeUnit.SECONDS.sleep(2);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                System.out.println(Thread.currentThread().getName()+"end:"+item+ "--" + LocalDateTime.now().toString());
            });
